...
function buildWorldMutatorSystemPrompt(language: "zh" | "en"): string {
  if (language === "en") {
    return [
      ...
      "Reuse the id of an entity that already exists in the current state: when the same person/thing reappears, UPDATE it under its existing id — never re-create it under a new id (or the panel shows the same person twice). An edge's fromId/toId MUST be entity ids (see the ids listed in the current state), not names.",
  ...
  return [
    ...
    "复用当前状态里已存在实体的 id：同一个人/物再次出现时，用它原来的 id 更新，绝不要换个新 id 重建（否则侧栏会出现重复的同一个人）。edges 的 fromId / toId 必须填实体的 id（见当前状态里列出的 id），不能填名字。",
